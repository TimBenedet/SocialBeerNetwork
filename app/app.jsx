...
function App() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api.me(token)
      .then(({ user }) => { if (cancelled) return; setActiveUser(user); setAuthed(true); checkinStore.hydrate().catch(()=>{}); })
  ...
  const handleLogin = (token, user) => {
    ...
    setAuthed(true);
    forceUser(n => n + 1);
    checkinStore.hydrate().catch(()=>{}); // pull this user's data from the server
  };
